...
# Main draw function
def draw(data, pgons, target, FPS, running, events, tr, goal, points):
    global save_data  
    ...
    # Draw the Goal
    pygame.draw.circle(screen, BLUE, to_screen_coords(goal[0], goal[1]), 8)

    # Draw lidar points
    if (len(points[0])>1):
        for i in range(points.shape[0]):
            pygame.draw.circle(screen, GREEN, to_screen_coords(points[i , 0], points[i , 1]), 5)
