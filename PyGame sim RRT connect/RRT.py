...
class RRTStar:
    ...
    def update_plot(self, Qs, Qg, newPoint=None, tree_color='blue', new_point_color='yellow'):
        """
        Updates the plot with new points and connections.
        """
        # Plot tree
        for Q in [Qs, Qg]:
            for point in Q:
                if int(point[2]) != 0:  # If it has a parent
                    parent = Q[int(point[2])]
                    self.ax.plot([point[0], parent[0]], [point[1], parent[1]], color=tree_color, alpha=0.7)

        # Highlight the new point
        if newPoint is not None:
            self.ax.plot(newPoint[0], newPoint[1], 'o', color=new_point_color, markersize=5)

        plt.pause(0.000000000001)  # Pause for animation
    def smooth(self, OptimalPath):
        ...
        for t in np.arange(0, len(OptimalPath), 0.1):
        ...
        while i < len(OptimalPath) - 1:
            ...
            for j in range(i + 2, len(OptimalPath)):
                ...
                for l in range(self.numobjects):
                    pgon = self.pgons[l]
                    shapely_pgon = Polygon(pgon.get_xy())
                    if line.intersects(shapely_pgon):
                        does_intersect = True
                        break
    ...
    def connect(self,Qs,Qg):
        ...
        for l in range(0,self.numobjects):
            line = LineString([Qs[row,:].flatten(),(Qg[col,:].flatten())])
            pgon = self.pgons[l]
            shapely_pgon = Polygon(pgon.get_xy())
    ...
    def find_path(self):
        N = 10
        start = time.time()
        Qs = np.array((self.start[0],self.start[1],0)).reshape((1,3)) # parent index [x y (point num)]
        Qg = np.array((self.goal[0],self.goal[1],0)).reshape((1,3)) # parent index [x y (point num)]
        ...
        for p in range(0,N):
            for l in range(0,self.numobjects):
                line = LineString([points[p,:].flatten(),(Qs[0,0],Qs[0,1])])
                pgon = self.pgons[l]
                shapely_pgon = Polygon(pgon.get_xy())
                does_intersect = line.intersects(shapely_pgon)
                if does_intersect:
        ...
        for p in range(0,N):
            for l in range(0,self.numobjects):
                line = LineString([points[p,:].flatten(),(Qg[0,0],Qg[0,1])])
                pgon = self.pgons[l]
                shapely_pgon = Polygon(pgon.get_xy())
                does_intersect = line.intersects(shapely_pgon)
                if does_intersect:
        ...
        for k in range(0,self.max_itterations,1):
            ...
            if not does_intersect:   
                ...
                #self.update_plot(Qs, Qg, newPoint=newPoint[0])
