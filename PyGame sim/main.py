...
def sample_points_along_polygon(pgon, x, y, num_samples=10):
    ...
    return distances,sampled_coords
...
data = {
    ...
    'heading': [ackerman.heading], 'speed': [0], 'left voltage': [0], 'right voltage': [0],'Closest object': [5], "l": [1]
}
    ...
    if running:
        ...
        for j in range(0, physics_updates_per_frame_update*frame_skip):
            if (j % physics_updates_per_controller_update == 0):
                ...
                closest_points, min_dist = detect_obstacles(ackerman.x, ackerman.y)    
                V_r, V_l,l = controller.update(ackerman.omega_l, ackerman.omega_r, ackerman.heading, ackerman.x, ackerman.y, path, closest_points)
        ...
        new_data = {
            ...
            'heading': ackerman.heading, 'speed': ackerman.speed, 'left voltage': V_l, 'right voltage': V_r , 'Closest object': min_dist, "l": l
        }
