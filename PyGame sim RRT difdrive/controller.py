...
class Controller:
    ...
    def __init__(self, Ts):
        ...
        class params():
            ...
            Y: set = {}
            sigma_f: float = 1
            length_scale: float = 0.75  # found from  loop demo will tune then adaptive
            ...
            u_max: float = np.array([2,10000]) # max accel, angle
            u_min: float = np.array([0.1,-10000]) # min accel, angle
    ...
    def update(self, omega_l, omega_r, phi, x, y, path, points):
        ...
        s_des = np.linalg.norm(U_GTG)
        phi_des = np.arctan2(U_GTG[1], U_GTG[0])
        #phi_des = np.arctan2(np.sin(phi_des), np.cos(phi_des))
        ...
        u = (self.CBFobj.constraints_cost([s_des , error],x,y,phi,np.linalg.norm(U_GTG)))
        
        #print('location')
        #print(x,y)

        if np.abs(np.linalg.norm(U_GTG/self.K_e)) < 0.2:
            u = [0,0]
